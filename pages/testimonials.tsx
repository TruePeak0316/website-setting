...
export default function TestimonialsPage() {
  ...
  return (
    <SiteLayout>
      <Seo title="客戶見證" path="/testimonials" description="整理客戶在 Google 地圖留下的服務評價，內容涵蓋稅務規劃、帳務處理、公司設立與日常溝通等合作經驗。" />
      ...
      <section className="testimonial-section section-pad overflow-hidden bg-brand-cream">
        <div className="page-shell">
          <div className="testimonial-summary relative overflow-hidden rounded-xs border border-brand-light/30 bg-white p-6 shadow-[0_18px_60px_rgb(7_86_111_/_0.08)] sm:p-8">
          ...
          <div className="mt-10 grid gap-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-stretch">
            <article className="testimonial-feature testimonial-card rounded-xs border border-brand-primary/25 bg-brand-dark p-6 text-white shadow-[0_22px_70px_rgb(7_86_111_/_0.16)] sm:p-8">
                ...
                <div ref={reviewRailRef} className="testimonial-review-rail no-scrollbar flex gap-4 pb-4 pt-1 md:grid md:grid-cols-2 md:overflow-visible md:pb-0">
                  {[...supportingReviews, ...supportingReviews, ...supportingReviews].map((review, index) => (
                    <article
                      ...
                      className={`testimonial-card testimonial-review-card w-[82vw] shrink-0 rounded-xs border border-brand-light/25 bg-white p-5 shadow-[0_14px_40px_rgb(7_86_111_/_0.06)] sm:w-[58vw] md:w-auto md:min-w-0 ${index < supportingReviews.length || index >= supportingReviews.length * 2 ? "md:hidden" : ""}`}
